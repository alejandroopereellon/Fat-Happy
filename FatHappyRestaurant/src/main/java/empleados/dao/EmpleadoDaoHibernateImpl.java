...
public class EmpleadoDaoHibernateImpl implements EmpleadosDao {
	...
	public boolean AlmacenarMovimientoEmpleado(MovimientosEmpleado movimiento) {
		...
		// Iniciamos una sesion
		try (Session session = HibernateUtil.getSessionFactory().openSession()) {
			logger.debug("Se ha iniciado una sesion de hibernate para persistir el movimiento del empleado");
			...
			transaction.commit();
			logger.debug("Se hecho commit del movimiento");

		} catch (Exception e) {
			logger.error("Ha ocurrido un error al persistir el movimiento del empleado", e);
