...
public class PanelCajaMetodos {
    ...
    public void rellenarDatosCaja() {
        ...
        if (caja != null && caja.getEmpleado() != null) {
            ...
            interfaz.getPuestoSesion()
                    .setText(ClasesEstaticas.getProveedorMensaje().findMessage("PANELCAJA_PUESTO") + caja.getNumeroCaja() + " | " + ClasesEstaticas.getProveedorMensaje().findMessage("PANELCAJA_SESION") + caja.getNumeroSesion());
    ...
    private void actualizarEstadoCaja(Caja caja) {
        ...
        if (caja != null && caja.getMomentoCierre() == null) {
            interfaz.getEstadoCaja().setText(ClasesEstaticas.getProveedorMensaje().findMessage("PANELCAJA_CAJA_ACTIVA"));
            logger.info("Se ha establecido el estado de la caja en abierta");
        } else {
            interfaz.getEstadoCaja().setText(ClasesEstaticas.getProveedorMensaje().findMessage("PANELCAJA_CAJA_CERRADA"));
    ...
    private void establecerInterfazNula() {
        // Establecemos el nombre y apellidos del empleado
        interfaz.getDatosEmpleado().setText(ClasesEstaticas.getProveedorMensaje().findMessage("PANELCAJA_NO_USUARIO_ACTIVO"));
        logger.debug("Se han establecido los datos del empleado en nulo");
        // Establecemos el puesto y sesion
        interfaz.getPuestoSesion().setText(ClasesEstaticas.getProveedorMensaje().findMessage("PANELCAJA_PUESTO") + ConfiguracionInicial.get().getNumeroCaja() + " | " + ClasesEstaticas.getProveedorMensaje().findMessage("PANELCAJA_SESION") + " 0");
